...
namespace NETAgents.Inference
{
    public class AzureOpenAIModel : Model
    {
        ...
        private IList<LLMMessage> _messageHistory;
        ...
        public AzureOpenAIModel(string modelId, string endpoint, string apiKey, bool cache = true) : base(modelId: modelId)
        {
            ...
            _messageHistory = new List<LLMMessage>();
        ...
        public override async Task<LLMMessage> GenerateAsync(List<LLMMessage> messages, ModelCompletionOptions? options = null,
                                               CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                LLMMessage? cachedResponse = LoadFromCache(cacheFilePath);
                ...
                LLMMessage response = ConvertFromAzureOpenAIResponse(completion);
        ...
        public override async Task<LLMMessage> Generate(List<LLMMessage> messages, ModelCompletionOptions? options = null)
        {
            return await GenerateAsync(messages, options);
        }

        public string GenerateCacheKey(List<LLMMessage> messages, ModelCompletionOptions? options)
        {
        ...
        private LLMMessage? LoadFromCache(string cacheFilePath)
        {
            try
            {
                ...
                return new LLMMessage(cacheData.Role, cacheData.Content, cacheData.Content, toolCalls)
                {
        ...
        private void SaveToCache(string cacheFilePath, LLMMessage response)
        {
            try
            {
        ...
        private static string? GetContentAsString(object? content)
        {
            switch (content)
            {
                ...
                case NETAgents.Models.AgentText agentText:
                    {
        ...
        private List<OpenAI.Chat.ChatMessage> ConvertToAzureOpenAIMessages(List<LLMMessage> messages)
        {
        ...
        private string GetMessageContent(LLMMessage message)
        {
        ...
        private List<OpenAI.Chat.ChatMessage> ConvertToOpenAIMessages(List<LLMMessage> messages)
        {
            List<OpenAI.Chat.ChatMessage> result = new List<OpenAI.Chat.ChatMessage>();
        ...
        private LLMMessage ConvertFromAzureOpenAIResponse(ChatCompletion completion)
        {
            string content = "";
        ...
        public override LLMMessage ParseToolCalls(LLMMessage message)
        {
            ...
            if (message.Content is string content)
            {
                try
                {
                    ...
                    {
                        return new LLMMessage(message.Role, message.Content, message.ContentString, toolCalls);
                    }
                }
                catch (Exception ex)
                {
